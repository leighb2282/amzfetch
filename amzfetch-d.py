...
# The app works as intended and can download the mp3s successfully (if that portion of the code wasn't commented out)
# But the GUI itself glitches out and doesn't remove previous entries from GUI, resulting in text 'funkiness'
...
def main(arguments):
    ...
    def openamz():
        ...
        if main.filename == '':
            ...
            itemfoo = itemfoo + 1
            dlurl = Button(root, text='Download Songs', command=urldl).grid(row=0, column=2, sticky=W)
    ...
    console.title('amzfetch-d.py :: Console Output')
    root.title('amzfetch-d.py ' + toolv + ' :: DEBUG ::')
    label = Label(console, text='amzfetch-d.py ' + toolv + ' :: DEBUG ::', fg="#357EC7").grid(sticky=W)
